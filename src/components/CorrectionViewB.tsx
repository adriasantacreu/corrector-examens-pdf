...
export default function CorrectionViewB({
    ...
    const renderPresetHighlighter = (preset: PresetHighlighter, shortcutKey?: string) => {
        ...
        const loadExerciseRegions = async () => {
            ...
            try {
                ...
                if (images.length > 0 && containerRef.current) {
                    ...
                    const padding = 40;
                    const spansTwoPages = currentExercise.type === 'pages' && (currentExercise as any).spansTwoPages;
                    const targetScaleX = (containerWidth - padding) / totalWidth;
                    const targetScaleY = (containerHeight - padding) / totalHeight;
                    // Scroll mode (vertical stack): fit by width → readable zoom, scroll down
                    // Side-by-side mode: fit both dimensions to show all pages at once
                    const targetScale = spansTwoPages
                        ? Math.min(targetScaleX, targetScaleY, 1.2)
                        : Math.min(targetScaleX, 1.2);
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            // Preset highlight shortcuts (1-9)
            const numKey = parseInt(e.key);
            if (!isNaN(numKey) && numKey >= 1 && numKey <= 9 && !e.ctrlKey && !e.metaKey) {
                const globalPresets = presets.filter(p => !p.exerciseId);
                const exPresets = presets.filter(p => p.exerciseId === currentExercise.id);
                const allVisible = [...globalPresets, ...exPresets];
                const target = allVisible[numKey - 1];
                if (target) {
                    setTool('highlighter');
        ...
        <div style={{ display: 'flex', width: '100%', flex: 1, flexDirection: 'column', minHeight: 0 }}>
            ...
            <div className="header" style={{ height: '70px', padding: '0 1.5rem', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 10, flexShrink: 0 }}>
                ...
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    {onBack && (
                        <button className="btn-icon" onClick={onBack} title="Tornar a Configuració">
                            <ChevronLeft />
                        </button>
                    )}
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <span className="correction-sidebar-label" style={{ marginBottom: 0 }}>Exercici</span>
                        <span style={{ fontSize: '1rem', fontWeight: 700 }}>{exerciseIdx + 1} / {gradableExercises.length}</span>
                    </div>
                ...
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '1.25rem', justifyContent: 'flex-end' }}>
                    <button onClick={onToggleTheme} className="btn-icon" title={isDarkMode ? 'Canviar a Mode Clar' : 'Canviar a Mode Fosc'}>
                    ...
                    {onFinish && (
                        <button onClick={onFinish} className="btn btn-primary" title="Finalitzar i enviar notes per correu">
                            <Send size={14} />
                            Finalitzar
                        </button>
                    )}
                ...
                <div className="grading-sidebar" style={{ width: '18rem', background: 'var(--bg-secondary)', borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column', flexShrink: 0, minHeight: 0 }}>
                        ...
                        return (
                            <div className="grade-top-card">
                                ...
                                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.4rem', flexWrap: 'wrap' }}>
                                    <span style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--accent)' }}>{roundedTotalStudentScore}</span>
                                    <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 600 }}>/ {totalPossibleReal} pt</span>
                                    <span style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--success)', marginLeft: 'auto' }}>= {scaledScore} / <NumericInput value={targetMaxScore} onChange={(v) => { if (v !== undefined) onUpdateTargetMaxScore(v); }} style={{ width: '32px', background: 'transparent', border: 'none', borderBottom: '1px solid var(--border)', color: 'var(--success)', fontSize: '0.8rem', fontWeight: 700, textAlign: 'center', padding: 0, display: 'inline' }} /></span>
                                </div>
                            </div>
                        );
                    })()}

                    {/* Student Section: OCR image on top, selector below */}
                    <div className="student-section">
                        {currentStudent?.nameCropUrl && (
                            <div style={{ position: 'relative', border: '1px solid var(--border)', borderRadius: '6px', overflow: 'hidden', background: 'white', display: 'flex', justifyContent: 'center' }}>
                                <img
                                    ...
                                    alt="Nom retallat OCR"
                                    style={{ height: '52px', width: 'auto', display: 'block', maxWidth: '100%', objectFit: 'contain' }}
                                ...
                                {currentStudent.originalOcrName && currentStudent.originalOcrName !== (currentStudent.name.split(' (')[0]) && (
                                    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(0,0,0,0.6)', color: 'white', fontSize: '10px', textAlign: 'center' }}>
                                        OCR?
                                    </div>
                                )}
                            </div>
                        ...
                        <div style={{ display: 'flex', gap: '0.25rem', alignItems: 'center' }}>
                            <button className="btn-icon" onClick={() => onUpdateStudentIdx(Math.max(0, studentIdx - 1))} disabled={studentIdx === 0} style={{ padding: '4px', flexShrink: 0 }}>
                                <ChevronLeft size={14} />
                            </button>
                            <select
                                value={studentIdx}
